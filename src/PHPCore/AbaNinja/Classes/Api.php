...
class Api
{
    ...
    private function request(
        ...
        try {
            ...
            switch ($e->getCode()) {
                ...
                case 401:
                    throw new AuthenticationException('Authentication failed. API-Token or API-Key invalid or expired',
                        401, $e);
                case 403:
     ...
     * @throws RuntimeException
     */
    public function __getActions(string|IApiModel $model, string $uuid): array
    {
        $response = $this->get($model::getResourceUri() . '/' . $uuid . '/actions');
        return (array)$response->getResponse()->data;
    }
     ...
     * @throws ApiException
     */
    public function __executeAction(string|IApiModel $model, string $uuid, string $action): ApiActionResponse
    {
        return ApiActionResponse::fromResponse(
            $this->patch($model::getResourceUri() . '/' . $uuid . '/actions/' . $action)
        );
    }
